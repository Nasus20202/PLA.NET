...
public class ProfessorServiceTests : ServiceTestBase
{
    ...
    {
        // Arrange
        _mockIndex.Setup(i => i.GetNextIndexAsync("P")).ReturnsAsync("P101");
        _mockRepo
    ...
    public async Task DeleteProfessorAsync_WhenExists_DeletesAndDecrements()
    {
        ...
        _mockIndex.Setup(i => i.TryDecrementIndexAsync("P", "P200")).ReturnsAsync(true);

        // Act
        await _service.DeleteProfessorAsync(1);

        // Assert
        _mockIndex.Verify(i => i.TryDecrementIndexAsync("P", "P200"), Times.Once);
